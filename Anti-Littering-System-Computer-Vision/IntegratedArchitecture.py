...
def detect_fines(frame, length, threshold, people):
    ...
    for person_df in people:
        ...
        # DeepFace.find() returns matches sorted by distance; take the closest one.
        # Its index isn't guaranteed to start at 0, so use positional access.
        person = person_df.iloc[0]

        # Retrieve the coordinates of the face bounding box
        x = person['source_x'] # x-co
        y = person['source_y'] # y-co
        w = person['source_w'] # Width
        h = person['source_h'] # height
